...
class mask_context(nn.Module):
    def __init__(self,edge_kernel = 3, conv_channel = 64, variance_kernel = 15, hidden_features = 2048, in_features = 256):
        ...
        self.conv_mask1 = nn.Conv2d(in_channels = 3, kernel_size = 3, stride = 1, out_channels = 1)
        self.conv_mask2 = nn.Conv2d(in_channels = 3, kernel_size = 3, stride = 1, out_channels = 1)
    ...
    def forward(self, img):
        ...
        std1m,_ = torch.max(std1.reshape(batch, -1), dim = -1, keepdims = True)
        std1 = self.conv_mask1(std1)
        std1 = torch.reshape(std1, (batch, -1))
        std2 = -mean2 *self.sig_factor2 / (std + 0.0001)
        std2 = self.sigmoid(std2)
        std2m,_ = torch.max(std2.reshape(batch, -1), dim = -1, keepdims = True)
        std2 = self.conv_mask2(std2)
        ...
        """
        x = torch.cat((std1, std2, std1m, std2m, #x
                        ), dim = -1)
        x = self.lin1(x)
        std = self.relu(x)
        std = self.lin2(std)
...
def get_activation(name):
    def hook(model, input, output):
        output_list.append(output.detach().cpu().numpy().reshape(-1, 32))
    return hook
...
class_counts = [400_000, 500*5, 200*10, 156*10]
...
a2 = 0.5#0.5#trial.suggest_float('a2',0.1, 0.9)
a3 = 0.9#0.675#trial.suggest_float('a3',0.1, 0.9)
a4 = 0.9#0.675#trial.suggest_float('a4',0.1, 0.9)
...
net = mask_context()
checkpoint = torch.load('augmented_final.tar', weights_only=True)
net.load_state_dict(checkpoint['model_state_dict'])
...
trainloader = DataLoader(trainset, batch_size = 32, sampler = sampler#shuffle = True #,sampler = sampler
                         , pin_memory = True)
testloader = DataLoader(testset, batch_size = 128, shuffle = False, pin_memory = True)
...
class combined_resnet(nn.Module):
    ...
    def forward(self, x):
        _, texture = self.texture(x)
        res = self.resnet(x)
        x = torch.cat([texture, res], dim = 1)
        combine = self.linear1(x)
        combine = self.relu(combine)
        combine = self.linear2(combine)
        return combine, x
...
torch.save({
            ...
            'loss': loss
            }, 'ensemble_resnet18_final.tar')
from sklearn.metrics import precision_recall_curve, auc
checkpoint = torch.load('ensemble_resnet18_final.tar', weights_only=True)

ensemble = combined_resnet().to(device)
ensemble.load_state_dict(checkpoint['model_state_dict'])

#ensemble.linear1.register_forward_hook(get_activation('linear1'))
trainset = image_load(train_ids, PATH, transform = False)
testset = image_load(test_ids, PATH, transform = False)
trainloader = DataLoader(trainset, batch_size = 128, shuffle = False, pin_memory = True)
testloader = DataLoader(testset, batch_size = 128, shuffle = False, pin_memory = True)
with autocast(enabled=True):
    ensemble.eval()
    output_list = []
    for i, (x, _) in enumerate(trainloader, 0):
        ...
        #x = rgb_to_lab(x)
        #y = y.to(device)
        _, predict = ensemble(x)
        batch = x.shape[0]
        output_list.append(predict.detach().cpu().numpy().reshape(batch, -1))
    train_features = np.vstack(output_list)
    output_list =[]
    for i, (x, _) in enumerate(testloader, 0):
        x = x.to(device)
        #x = rgb_to_lab(x)
        #y = y.to(device)
        _, predict = ensemble(x)
        batch = x.shape[0]
        output_list.append(predict.detach().cpu().numpy().reshape(batch, -1))
    test_features = np.vstack(output_list)

train_x = np.hstack((train_x, train_features))
...
print('auc roc')
print(roc_auc_score(test_y, malignant_chance['malignant']))
print('partial_auc')
print(score(test_y, malignant_chance))
print('pr auc')
print(pr_auc(test_y, malignant_chance))

import joblib
joblib.dump(classifier, 'ensemble_svm.pkl')


classifier = SVC(kernel = 'rbf', C = 0.1, cache_size = 2000, class_weight = None, probability = True)
classifier.fit(train_x, train_y)
...
malignant_chance = pd.DataFrame(np.sum(np.array(predicted)[:, 2:4], axis =1))
print('rbf not balanced')
print('partial_auc')
print(score(test_y, malignant_chance))
print('pr auc')
print(pr_auc(test_y, malignant_chance))

classifier = SVC(kernel = 'linear', C = 0.1, cache_size = 2000, class_weight = None, probability = True)
classifier.fit(train_x, train_y)
...
print('linear not balanced')
print('partial_auc')
print(score(test_y, malignant_chance))
print('pr auc')
print(pr_auc(test_y, malignant_chance))

classifier = SVC(kernel = 'linear', C = 0.1, cache_size = 2000, class_weight = 'balanced', probability = True)
classifier.fit(train_x, train_y)
